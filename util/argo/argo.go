...
func ValidateRepo(
	...
	if repoValidateDur > 10*time.Second {
		logCtx.WithFields(log.Fields{"duration_ms": repoValidateDur.Milliseconds()}).Info("validateRepo (TestRepository + GenerateManifest) exceeded 10s")
	}
	if err != nil {
		return nil, err
	}
...
func verifyGenerateManifests(
	...
	for _, source := range sources {
		...
		log.WithFields(log.Fields{
			...
			"debugTag":    "argo504debug",
		}).Info("GenerateManifest gRPC call to repo-server completed")
		if genDur > 10*time.Second {
			log.WithFields(log.Fields{
				...
				"duration_ms": genDur.Milliseconds(),
				"debugTag":    "argo504debug",
			}).Info("GenerateManifest gRPC call to repo-server exceeded 10s, risk of ALB 504")
		}
		if err != nil {
