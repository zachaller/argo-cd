...
func (m *appStateManager) CompareAppState(ctx context.Context, app *v1alpha1.Application, project *v1alpha1.AppProject, revisions []string, sources []v1alpha1.ApplicationSource, noCache bool, noRevisionCache bool, localManifests []string, hasMultipleSources bool) (_ *comparisonResult, retErr error) {
	...
	conditions = append(conditions, m.warnOnDivergentCapabilities(resolvedDests, destOrder, now)...)
	...
	app.Status.SetConditions(conditions, map[v1alpha1.ApplicationConditionType]bool{
		...
		// Listed so the warning clears once the destinations agree on a version again.
		v1alpha1.ApplicationConditionMultipleDestinationsWarning: true,
	})
