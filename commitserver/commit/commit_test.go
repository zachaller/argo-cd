...
func Test_CommitHydratedManifests(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		...
		mockGitClient.EXPECT().GetCommitNote(mock.Anything, mock.Anything).Return("", fmt.Errorf("test %w", err)).Once()
		mockGitClient.EXPECT().AddAndPushNote(mock.Anything, mock.Anything, mock.Anything).Return(nil).Once()
		mockGitClient.EXPECT().CommitSHA().Return("abc123-sha", nil).Once()
